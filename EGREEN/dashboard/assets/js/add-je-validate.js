$(document).ready(function(){
  ...
  var npsharejeid=$('#jentnpid');
  var inpsharejeid=$('#jentinpid');
  var npsharebtnid=$('#shasnpid');
  var inpsharebtnid=$('#shasinpid');  
  ...
  // validation when clicking share as normal post button
    $("#shasnpid").click(function(e){
    e.preventDefault();
    var jentrid = npsharejeid.val();
    // var img_path = tleimage.val();
    if(confirm('Are you sure to share this entry as a normal post ?')) {
      $.post('set-normal-post.php', {id:jentrid}, function(data){
        if(data!=0){
          alert ("Journal entry has been shared as a normal post successfully.");
          // $('#blah').attr('src', "../images/tlimg/author-img1.jpg");
          // location.reload();
          // state = false;
        } 
        else {
          alert ("Journal entry cannot be shared or already shared, as a normal post.");
        }
      });  
    } 
    else {
  ...
  // validation when clicking share as instruction needy post button
